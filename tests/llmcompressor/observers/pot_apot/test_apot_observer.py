"""
Unit tests for Additive Power-of-Two Observer.
"""
import pytest
import torch
...
class TestAPoTObserver:
    """Test cases for APoT Observer."""
    
    def test_apot_observer_creation(self):
        """Test that APoT observer can be created."""
        quant_args = QuantizationArgs(
            num_bits=4,
            symmetric=True,
            type="int",
        )
        observer = APoTObserver(quant_args, num_terms=2)
        assert observer.quantization_args.num_bits == 4
        assert observer.quantization_args.symmetric is True
        assert observer.num_terms == 2
    
    def test_apot_observer_default_num_terms(self):
        """Test that APoT observer uses default num_terms."""
        quant_args = QuantizationArgs(
            num_bits=4,
            symmetric=True,
            type="int",
        )
        observer = APoTObserver(quant_args)
        assert observer.num_terms == 2  # default value
    ...
    def test_calculate_num_apot_levels(self):
        """Test calculation of APoT quantization levels."""
        quant_args = QuantizationArgs(
            num_bits=4,
            symmetric=True,
            type="int",
        ...
        # test with 2-term APoT
        num_levels_2term = observer._calculate_num_apot_levels(4, 2)
        assert num_levels_2term > 2**3  # should have more levels than simple PoT
        
        # test with different bit widths
        num_levels_3bit = observer._calculate_num_apot_levels(3, 2)
        num_levels_5bit = observer._calculate_num_apot_levels(5, 2)
        
        # more bits should give more levels
        assert num_levels_5bit > num_levels_3bit
    
    def test_different_num_terms(self):
        """Test observer with different numbers of terms."""
        quant_args = QuantizationArgs(
            num_bits=4,
            symmetric=True,
            type="int",
        ...
        for num_terms in [1, 2, 3]:
            observer = APoTObserver(quant_args, num_terms=num_terms)
    ...
    def test_global_scale_application(self):
        ...
        # the resulting scale should be positive
        assert torch.all(scale > 0)
    ...
    def test_scale_is_power_of_two_after_rounding(self):
        """Test that calculated scales become powers of two after rounding."""
        quant_args = QuantizationArgs(
            num_bits=4,
            symmetric=True,
            type="int",
        ...
        tensor = torch.tensor([3.7, 7.2, 15.8])
        scale, zero_point = observer.calculate_qparams(tensor)
        
        # after the internal rounding, scale should be a power of two
